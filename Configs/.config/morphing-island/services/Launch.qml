...
// The single place that starts programs from the island (apps, files, URLs, picker actions) and
// pastes text into the focused window.
//
// Programs must not die with the island. The island runs in a systemd unit (ExitType=cgroup): a
// plain execDetached stays in the island's cgroup and is killed when it restarts. With
...
Singleton {
    ...
    // PickersConfig.pasteDelayMs for the launcher to close and focus to return to the previous window,
    ...
    function pasteWith(copyScript, arg) {
        const delay = Math.max(0, Config.PickersConfig.pasteDelayMs) / 1000;
