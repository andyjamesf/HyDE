...
// pickers.json in the shell's state folder (Paths.shellState). One FileView/JsonAdapter for all
...
Singleton {
    ...
    // Puts `value` at the head of list `key` (no duplicates, at most Config.pickers.recents[key] items)
    // and saves.
    function push(key, value) {
        if (!["emoji", "glyph", "bookmarks"].includes(key) || value === undefined || value === null || value === "")
            return;
        const max = Config.pickers.recents[key] ?? 40;
        const cur = store[key] ?? [];
    ...
    FileView {
        id: file
        path: `${Paths.shellState}/pickers.json`
        printErrors: false
        onLoadFailed: error => {
